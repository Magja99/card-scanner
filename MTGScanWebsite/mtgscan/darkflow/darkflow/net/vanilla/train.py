...
_LOSS_TYPE = ["sse", "l2", "smooth", "sparse", "l1", "softmax", "svm", "fisher"]


def loss(self, net_out):
    m = self.meta
    loss_type = self.meta["type"]
    assert loss_type in _LOSS_TYPE, "Loss type {} not implemented".format(loss_type)
    ...
    if loss_type in ["sse", "12"]:
        ...
        assert "train_size" in m, "Must specify"
        size = m["train_size"]
