...
class connected(BaseOp):
    def forward(self):
        self.out = tf.nn.xw_plus_b(
            self.inp.out, self.lay.w["weights"], self.lay.w["biases"], name=self.scope
        )
...
class leaky(BaseOp):
    def forward(self):
        self.out = tf.maximum(0.1 * self.inp.out, self.inp.out, name=self.scope)
