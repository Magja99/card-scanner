...
class local_layer(Layer):
    def setup(self, ksize, c, n, stride, pad, w_, h_, activation):
        ...
        self.wshape = dict(
            {"biases": [h_ * w_ * n], "kernels": [h_ * w_, ksize, ksize, c, n]}
        )
...
class conv_extract_layer(Layer):
    def setup(self, ksize, c, n, stride, pad, batch_norm, activation, inp, out):
        ...
        self.out = out
        self.wshape = dict(
            {"biases": [len(out)], "kernel": [ksize, ksize, len(inp), len(out)]}
        )
    ...
    def recollect(self, w):
        ...
        assert assert1 and assert2, "Dimension not matching in {} recollect".format(
            self._signature
        )
        self.w["kernel"] = k
        self.w["biases"] = b
...
class convolutional_layer(Layer):
    def setup(self, ksize, c, n, stride, pad, batch_norm, activation):
        ...
        if self.batch_norm:
            self.wshape.update(
                {"moving_variance": [n], "moving_mean": [n], "gamma": [n]}
            )
            self.h["is_training"] = {"feed": True, "dfault": False, "shape": ()}
