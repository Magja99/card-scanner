class argHandler(dict):
    ...
    def setDefaults(self):
        ...
        self.define(
            "annotation", "../pascal/VOCdevkit/ANN/", "path to annotation directory"
        )
        ...
        self.define(
            "load",
            "",
            "how to initialize the net? Either from .weights or a checkpoint, or even from scratch",
        )
        ...
        self.define(
            "metaLoad",
            "",
            "path to .meta file generated during --savepb that corresponds to .pb file",
        )
        ...
        while i < len(args):
            ...
            if isinstance(self.get(argumentName), bool):
                if (
                    not (i + 1) >= len(args)
                    and (
                        args[i + 1].lower() != "false" and args[i + 1].lower() != "true"
                    )
                    and not args[i + 1].startswith("--")
                ):
                    print(
                        "ERROR - Expected boolean value (or no value) following argument: "
                        + args[i]
                    )
                    print("Try running flow --help")
                    exit()
                elif not (i + 1) >= len(args) and (
                    args[i + 1].lower() == "false" or args[i + 1].lower() == "true"
                ):
